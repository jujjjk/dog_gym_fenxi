...
class FanfanRouheStraightRobot(LeggedRobot):
    ...
    def _init_buffers(self):
        ...
        for body_index in self.feet_indices.cpu().tolist():
            ...
            phase_offsets.append(0.0 if name.startswith("FL_") or name.startswith("RR_") else 0.5)
        self.gait_phase_offsets = torch.tensor(phase_offsets, dtype=torch.float, device=self.device)
    ...
    def _compute_torques(self, actions):
        ...
        actions_scaled[:, self.rear_sagittal_dof_indices] = (
            actions[:, self.rear_sagittal_dof_indices] * self.cfg.control.rear_action_scale
        )
        actions_scaled[:, self.hip_dof_indices] = (
            actions[:, self.hip_dof_indices] * self.cfg.control.hip_action_scale
        )

        phase = (self.gait_phase.unsqueeze(1) + self.gait_phase_offsets.unsqueeze(0)) % 1.0
    ...
    def _post_physics_step_callback(self):
        ...
        # 直行纠偏用固定 yaw 偏置。放在 super() 后面，避免被 legged_gym 的
        # heading controller 覆盖。默认 0.0；真机固定一边偏时再从 ±0.02 试。
        yaw_bias = getattr(self.cfg.commands, "command_yaw_bias", 0.0)
        if abs(float(yaw_bias)) > 1.0e-9:
            max_abs_yaw = getattr(self.cfg.commands, "max_abs_yaw_rate_command", 0.6)
            self.commands[:, 2] = torch.clip(self.commands[:, 2] + yaw_bias, -max_abs_yaw, max_abs_yaw)
    ...
    def _reward_heading_error(self):
        ...
        return torch.square(wrap_to_pi(target_yaw - yaw))

    def _reward_world_y_drift(self):
        if hasattr(self, "env_origins"):
            y0 = self.env_origins[:, 1]
        else:
            y0 = 0.0
        return torch.square(self.root_states[:, 1] - y0)
    ...
    def _get_desired_foot_contacts(self):
        ...
        desired[:] = ((self.gait_phase.unsqueeze(1) + self.gait_phase_offsets.unsqueeze(0)) % 1.0) < stance_ratio
        return desired
    ...
    def _reward_swing_height(self):
        ...
        height_error = torch.square(self.feet_pos[:, :, 2] - self.cfg.rewards.swing_height_target)
    ...
    def _reward_rear_leg_posture(self):
        ...
        posture_error = torch.sum(
            torch.square(self.dof_pos[:, rear_leg_indices] - self.default_dof_pos[:, rear_leg_indices]),
            dim=1,
        )
